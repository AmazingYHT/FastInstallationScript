...
create_user_and_dirs() {
    ...
    if [ -d "$MYSQL_DATA_DIR" ] && [ "$(ls -A $MYSQL_DATA_DIR 2>/dev/null)" ]; then
        ...
        if [[ "$BATCH_MODE" == "1" ]]; then
            if [[ "$FORCE_INIT_DATA" == "1" ]]; then
                echo -e "${YELLOW}无人值守模式 + --force-init-data，直接清空数据目录${NC}"
                rm -rf "${MYSQL_DATA_DIR:?}"/*
            else
                echo -e "${YELLOW}无人值守模式：保留现有数据目录${NC}"
            fi
        else
        echo "1. 保留现有数据目录"
        echo "2. 备份后清空"
...
EOSQL
    ...
    if [ "$password_set" = false ]; then
        ...
        if [[ "$BATCH_MODE" != "1" ]]; then
            ...
            echo -e "${YELLOW}无人值守模式：跳过等待，请稍后检查 root 密码${NC}"
        fi
    fi
...
无人值守模式（主从脚本/SSH远程调用）:
  bash install_mysql.sh --batch --version 8.4.9 --home /mnt/data/mysql \
      --port 3306 --password root

  bash install_mysql.sh --batch --offline /path/mysql-xxx.tar.xz \
      --home /mnt/data/mysql --port 3306 --password root
...
apply_batch_defaults() {
    ...
    MYSQL_GROUP="${MYSQL_GROUP:-$DEFAULT_MYSQL_GROUP}"
    MYSQL_HOME="${MYSQL_HOME:-$DEFAULT_MYSQL_HOME}"
...
# Generated by install_mysql.sh on $(date)
...
EOF
    echo -e "${GREEN}✓ 安装状态已写入 $INSTALL_STATE_FILE${NC}"
}
...
batch_install_flow() {
    ...
    echo -e "${GREEN}=====================================${NC}"
    echo -e "${GREEN}MySQL 无人值守安装${NC}"
    echo -e "${GREEN}=====================================${NC}"
    echo -e "  版本:     $MYSQL_VERSION"
    echo -e "  安装目录: $MYSQL_INSTALL_DIR"
    echo -e "  数据目录: $MYSQL_DATA_DIR"
    echo -e "  端口:     $MYSQL_PORT"
    echo -e "  离线包:   ${OFFLINE_TARBALL_PATH:-在线下载}"
    echo ""
    ...
    if [[ "$OFFLINE_MODE" == "1" ]]; then
        if [ ! -f "$OFFLINE_TARBALL_PATH" ]; then
            echo -e "${RED}离线安装包不存在: $OFFLINE_TARBALL_PATH${NC}"
            return 1
        fi
        # 解压前先复制到 /tmp 便于后续 scp
        local stage_name
        stage_name=$(basename "$OFFLINE_TARBALL_PATH")
        if [ "$OFFLINE_TARBALL_PATH" != "/tmp/$stage_name" ]; then
            cp -f "$OFFLINE_TARBALL_PATH" "/tmp/$stage_name"
        fi
        parse_tarball_glibc_pkg "/tmp/$stage_name"
        OFFLINE_TARBALL_PATH="/tmp/$stage_name"
        extract_offline_tarball || return 1
    else
        # batch 在线安装默认保留包，供主从脚本 scp 到远程
        KEEP_TARBALL=1
        download_binary || return 1
        extract_and_install || return 1
    fi
    ...
    if [[ "$KEEP_TARBALL" != "1" ]]; then
        ...
        echo -e "${CYAN}安装包已保留: /tmp/${TARBALL_NAME}${NC}"
    fi

    echo -e "${GREEN}无人值守安装完成${NC}"
    return 0
}
