...
remote_install_kafka() {
    ...
    if [ "$has_java" != "YES" ]; then
        ...
        ssh_cmd "$host" "$(declare -f detect_sys_pkg sys_pkg_install); detect_sys_pkg; if [ \"\$SYS_FAMILY\" = debian ]; then apt-get update -y; fi; sys_pkg_install 'java-17-openjdk' 'openjdk-17-jdk'" || true
        has_java=$(ssh_cmd "$host" "command -v java >/dev/null && echo YES || echo NO")
        if [ "$has_java" != "YES" ]; then
